...
class StockMarket:
    ...
    def next_day(self) -> Dict[str, float]:
        self.current_day += 1
        self._update_regime()
        # Add a tiny jitter to ensure positive definiteness
        ...
        for i, (symbol, stock) in enumerate(self.stocks.items()):
            ...
            jump = np.random.normal(stock['jump_params']['mean'], stock['jump_params']['std']) if np.random.rand() < stock['jump_params']['prob'] else 0
...
class HistoricalMarket:
    """
    Historical market that downloads price data via yfinance and “plays back” the data one day at a time.
    Mimics the StockMarket interface.
    """
    def __init__(self, tickers: List[str], start_date: str, end_date: str, risk_free_rate: float = 0.02):
        ...
        for ticker in tickers:
            ...
            if df.empty:
                ...
                continue
            # Normalize column names in case they are multi-indexed
            ...
            if 'Adj Close' in df.columns:
                ...
                continue
            # Ensure we have a Series (if not, take the first column)
            if isinstance(price_series, pd.DataFrame):
                price_series = price_series.iloc[:, 0]
            self.data[ticker] = price_series.values.tolist()
            self.tickers.append(ticker)
            if self.dates is None:
                self.dates = df.reset_index()['Date']
        if not self.tickers:
            st.error("No valid tickers found. Please check your input.")
            st.stop()
        self.current_index = 0
        self.total_days = len(self.dates)
        self.histories = {ticker: [self.data[ticker][0]] for ticker in self.tickers}
    ...
    def get_prices(self) -> Dict[str, float]:
        return {ticker: self.data[ticker][self.current_index] for ticker in self.tickers}
...
class Portfolio:
    ...
    def equity(self) -> float:
        ...
        for contracts in self.options.values():
            for contract in contracts:
                ...
                price = self._black_scholes(S, contract['strike'], T,
    ...
    def place_order(self, symbol: str, qty: float, option_contract: Dict = None):
        ...
        if cost > self.margin_available() + self.cash:
            ...
            return
        self.cash -= cost
        self.trade_history.append({'day': self.current_day, 'symbol': symbol, 'qty': qty, 
...
class VolatilityTargetStrategy:
    ...
    def execute(self, market, portfolio: Portfolio):
        ...
        for symbol in symbols:
            ...
            target_value = portfolio.equity() * leverage / len(symbols)
    ...
    def execute(self, market, portfolio: Portfolio):
        ...
        if hasattr(market, 'stocks'):
            for symbol, data in market.stocks.items():
                ...
                sector_returns[data['sector']].append(ret)
        else:
...
class TrendFollowingStrategy:
    ...
    def execute(self, market, portfolio: Portfolio):
        ...
        symbols = market.tickers if hasattr(market, 'tickers') else list(market.stocks.keys())
        for symbol in symbols:
            history = market.get_history(symbol)[-self.long_window:]
...
class OptionsStrategy:
    ...
    def execute(self, market, portfolio: Portfolio):
        ...
        for symbol in symbols:
            ...
            sigma = (market.stocks[symbol]['volatility'] if hasattr(market, 'stocks') else 0.2)
...
def main():
    ...
    if market_mode == "Historical Data":
        # In historical mode, provide two separate Buy & Hold strategies.
        strategies = {
            "Buy & Hold (Selected Stocks)": None,
            "Buy & Hold (S&P 500)": None,
    ...
    if st.sidebar.button("Run Backtest"):
        ...
        results = []
        # For Simulated mode, use one market instance.
        if market_mode == "Simulated Market":
            market = StockMarket(n_stocks=n_stocks)
            total_days = years * 252
            # Create one portfolio per strategy.
            ...
            for day in range(total_days):
                ...
                for name in selected_strategies:
                    ...
                    if name == "Buy & Hold" and day == 0:
                        ...
                        for symbol in symbols:
                            price = prices[symbol]
                            qty = (portfolio.equity() / len(symbols)) / price
            ...
            # Historical Mode: create separate market instances per strategy.
            # For strategies that run on user-selected stocks:
            market_selected = HistoricalMarket(tickers=tickers, start_date=str(start_date), end_date=str(end_date))
            # For the S&P 500 Buy & Hold strategy:
            market_spy = HistoricalMarket(tickers=["SPY"], start_date=str(start_date), end_date=str(end_date))
            # Determine total days as the minimum across markets.
            total_days = min(market_selected.total_days, market_spy.total_days)
            portfolios = {}
            markets = {}
            strategy_instances = {}
            for name in selected_strategies:
                portfolios[name] = Portfolio(initial_capital)
                portfolios[name].current_day = 0
                if name == "Buy & Hold (S&P 500)":
                    markets[name] = market_spy
                else:
                    markets[name] = market_selected
                if strategies[name] is not None:
                    strategy_instances[name] = strategies[name]
            for day in range(total_days):
                for name in selected_strategies:
                    market_inst = markets[name]
                    prices = market_inst.next_day()
                    portfolio = portfolios[name]
                    portfolio.update_prices(prices)
                    portfolio.current_day = day
                    if (name.startswith("Buy & Hold")) and day == 0:
                        # For Buy & Hold strategies, invest equally.
                        if name == "Buy & Hold (S&P 500)":
                            symbols = market_inst.tickers  # should be ['SPY']
                        else:
                            symbols = market_inst.tickers
                        for symbol in symbols:
                            price = prices[symbol]
                            qty = (portfolio.equity() / len(symbols)) / price
                            portfolio.place_order(symbol, qty)
                    elif name in strategy_instances:
                        strategy_instances[name].execute(market_inst, portfolio)
                    portfolio.daily_update(day)
                    results.append({"day": day, "strategy": name, "value": portfolio.equity()})
                progress_bar.progress((day + 1) / total_days)
                status_text.text(f"Day {day + 1} of {total_days}")
